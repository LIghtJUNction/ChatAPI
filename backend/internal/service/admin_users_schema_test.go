...
func TestBuildAdminUsersSchema(t *testing.T) {
	...
	if schema.Operations[7].Name != "unlink_user_identity" {
		t.Fatalf("unexpected admin users unlink identity schema: %#v", schema.Operations[7])
	}
	if schema.Operations[8].Name != "deactivate_user" {
		t.Fatalf("unexpected admin users deactivate schema: %#v", schema.Operations[8])
	}
	if schema.Operations[9].Name != "purge_user" {
		t.Fatalf("unexpected admin users purge schema: %#v", schema.Operations[9])
	}
}
