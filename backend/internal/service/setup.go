...
var ErrSetupEnvExists = errors.New(".env already exists")
...
func NewSetupService(dataStore store.Store, cfg config.Config) *SetupService {
	return &SetupService{store: dataStore, cfg: cfg}
}
...
func (s *SetupService) Status(ctx context.Context) (SetupStatus, error) {
	...
	if s == nil || s.store == nil {
		...
		return status, ErrSetupUnavailable
	}
...
func (s *SetupService) Run(ctx context.Context, input SetupApplyInput) (SetupReport, error) {
	...
	template, err := BuildSetupEnvTemplate(strings.TrimSpace(input.AdminPassword))
	if err != nil {
		report.OK = false
	...
	if status.ExistingEnv && !input.Force {
		report.OK = false
		report.Error = ErrSetupEnvExists.Error()
		return report, ErrSetupEnvExists
	}
...
func BuildSetupEnvTemplate(adminPassword string) (string, error) {
	adminPassword = strings.TrimSpace(adminPassword)
	if adminPassword == "" {
		value, err := randomURLTokenService(24)
		if err != nil {
			return "", err
		}
		adminPassword = value
	}
	masterKey, err := randomURLTokenService(48)
	if err != nil {
		return "", err
	}
	sessionSecret, err := randomURLTokenService(32)
	if err != nil {
		return "", err
	}
...
func (s *SetupService) hasAdminAccess(ctx context.Context) (bool, error) {
...
func randomURLTokenService(bytesLen int) (string, error) {
	if bytesLen <= 0 {
