...
- 管理员用户管理已落地最小接口：`GET /api/admin/users` 列出本地用户，`POST /api/admin/users` 创建本地用户并写入 Argon2id 密码 hash，`PUT /api/admin/users/{user_id}/password` 重置密码，`DELETE /api/admin/users/{user_id}` 做停用，`GET /api/admin/users/{user_id}/delete-preview` 返回物理删除预检，`POST /api/admin/users/{user_id}/transfer-ownership` 可把阻断 purge 的 conversations / uploaded_images / 删除失败队列 owner 转移给另一个用户，`GET /api/admin/users/{user_id}/ownership-items` 可列出当前仍归属于该用户的 conversations 和 uploads，`POST /api/admin/users/{user_id}/transfer-ownership-selection` 可只迁移选中的 conversation ids / upload filenames，`POST /api/admin/users/{user_id}/cleanup-selection` 可直接删除选中的 closed conversations 和不再被剩余历史引用的 uploads，`POST /api/admin/users/{user_id}/purge` 在无阻断项时执行物理删除；当前 purge 会删除 identities、user configs、automation rules、应用/虚拟模型 API Key、应用 API Key 审计明细、存储配额和删除失败队列，但会保留审计日志，并在存在 owned conversations / uploaded images 时返回 `409` 阻断，避免把仍需保留的历史 owner 记录静默变成孤儿数据。ownership transfer 当前不会转移账号直接归属的 key/config，而是只处理 purge blocker 和紧邻的存储归属；如果 source 有 quota override 且 target 没有，则 quota override 一并迁移，否则保留 target quota 并删除 source override。局部 transfer 当前不会触碰 quota override，只做精细的历史 owner 收口；局部 cleanup 会显式跳过 waiting/streaming 活跃会话和仍被保留会话引用的图片，而不是强删造成历史断链。这些接口仅允许 admin session 访问，并写入 `admin.user` 审计事件。
...
- 用户配置、系统配置、虚拟模型 API Key、应用 API Key、上游模型辅助的浏览器本地配置、KirariNetwork 连接、管理员用户管理。当前用户配置已先支持 `GET/POST /api/user/config` 按当前 actor 读写普通 JSON object 偏好；系统配置已先支持 `GET/POST /api/admin/config` 做表驱动持久化；管理员用户管理已先支持列表、创建、查看历史、查看/解绑 OIDC 身份、重置密码、停用用户，以及 `delete-preview + transfer-ownership + purge` 三段式物理删除策略。当前 purge 仅删除账号直接归属的数据行，并显式保留审计日志；如果用户仍拥有 conversations 或 uploaded images，可先通过 transfer-ownership 把历史 owner 归属迁给其他用户，再执行 purge。后续仍可继续扩展成更细的“按会话/按上传文件局部转移或局部清理”策略。
