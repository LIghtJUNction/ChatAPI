...
- `stream=true` 已开始走真实 SSE 链路，不再只支持等待最终非流式结果；当前已覆盖 OpenAI Responses 与 Chat Completions 的最小流式集成测试。
...
- 现有 `tests/` SDK 脚本通过。
- 前端人工接管完整链路通过。
- Go `httptest` 集成测试至少覆盖三套协议的非流式闭环，并覆盖 Responses / Chat Completions 的 `stream=true` 基础 SSE 行为。
