...
- `GET /lab/requests/schema` 和 `GET /api/app/requests/schema` 当前也已开始显式暴露 `parsed_item_fields`、`parsed_detail_fields` 和 `replay_fields` 三组字段定义，把 request list/detail 里的解析视图与 replay 结构本身做成机器可读契约，避免工作台、CLI 和未来独立调试工具继续从示例响应里反推 `request_method`、`request_headers`、`normalized_tool_schemas`、`replay.curl` 这类字段。
...
- 应用 API 契约当前已进一步补成机器可读矩阵：`GET /api/user/app-api-keys/schema` 除 scopes / resource limit 外，还会返回 app API 鉴权头格式、统一错误码、每个 `/api/app/*` 操作的路径 / scope / 响应形状 / 受哪些 resource limit 影响；`GET /api/app/me|requests|conversations|statistics|model-keys|automation-rules/schema` 也会分别暴露认证头、错误码、rate limit 语义和 route contract，避免外部自动化工具继续靠文档或抓包反推 `Authorization` / `X-ChatAPI-App-Key`、`429 rate_limited`、`allowed_request_ids`、`allowed_automation_rule_ids`、`max_model_keys` 这类约束。
...
- 虚拟模型配置契约当前也已开始显式暴露：`GET /api/config/models/schema` 会返回创建字段定义，避免前端继续硬编码 `id/name/owned_by/created/enabled` 这组字段；`POST /api/config/models` 对缺失 `id` 或 `name` 的请求会返回字段级 `400`，而不是统一的 `invalid virtual model`。
...
- 管理员运行时操作 schema 已开始显式暴露：`GET /api/admin/runtime/schema` 当前返回 automation diagnostics、runtime settings、force GC 这几类写/查询操作的字段定义、默认值、路径和说明，避免前端或 CLI 继续硬编码 `limit`、`reason`、`rule_id`、`gogc`、`memory_limit_bytes` 以及“GC 接口不接收 body”这类约束。
...
- 虚拟模型协议入口当前也已接入同一套 realtime 连接池：非流请求在等待人工/自动完成期间占用 `api` lease，SSE 流式请求占用 `sse` lease；超限时返回协议原生 `429 rate_limit_error` / `connection_limit_exceeded`，同时仍保留每用户的 WebUI 预留名额，避免挂起请求把浏览器控制台挤掉。
...
- 管理员用户管理已落地最小接口：`GET /api/admin/users` 列出本地用户，`POST /api/admin/users` 创建本地用户并写入 Argon2id 密码 hash，`PUT /api/admin/users/{user_id}/password` 重置密码，`DELETE /api/admin/users/{user_id}` 做停用，`GET /api/admin/users/{user_id}/delete-preview` 返回物理删除预检，`GET /api/admin/users/{user_id}/delete-overview` 返回聚合删除总览（预检 + owned history items + recommended next actions），`POST /api/admin/users/{user_id}/transfer-ownership` 可把阻断 purge 的 conversations / uploaded_images / 删除失败队列 owner 转移给另一个用户，`GET /api/admin/users/{user_id}/ownership-items` 可列出当前仍归属于该用户的 conversations 和 uploads，`POST /api/admin/users/{user_id}/transfer-ownership-selection` 可只迁移选中的 conversation ids / upload filenames，`POST /api/admin/users/{user_id}/cleanup-selection` 可直接删除选中的 closed conversations 和不再被剩余历史引用的 uploads，`POST /api/admin/users/{user_id}/purge` 在无阻断项时执行物理删除；当前 purge 会删除 identities、user configs、automation rules、应用/虚拟模型 API Key、应用 API Key 审计明细、存储配额和删除失败队列，但会保留审计日志，并在存在 owned conversations / uploaded images 时返回 `409` 阻断，避免把仍需保留的历史 owner 记录静默变成孤儿数据。`delete-overview` 现在可以作为前端管理员删除流程的单一入口，先读总览，再决定是局部 transfer、局部 cleanup 还是直接 purge。ownership transfer 当前不会转移账号直接归属的 key/config，而是只处理 purge blocker 和紧邻的存储归属；如果 source 有 quota override 且 target 没有，则 quota override 一并迁移，否则保留 target quota 并删除 source override。局部 transfer 当前不会触碰 quota override，只做精细的历史 owner 收口；局部 cleanup 会显式跳过 waiting/streaming 活跃会话和仍被保留会话引用的图片，而不是强删造成历史断链。这些接口仅允许 admin session 访问，并写入 `admin.user` 审计事件。
...
- 管理员用户管理 schema 已开始显式暴露：`GET /api/admin/users/schema` 当前声明本地用户列表、创建、查看历史、查看已绑定身份、删除预检、全量 ownership transfer、ownership item 列表、局部 ownership transfer、局部 cleanup、重置密码、解绑身份、停用用户、物理删除这些操作的路径、字段、默认值以及“停用”“transfer”“cleanup”“purge”的不同语义，避免前端或 CLI 再各自硬编码 `role` 枚举、`history.limit` 默认值和请求体字段。
...
- 本地 session 已落地最小版本：serve 模式下 `POST /api/auth/login` 优先验证 `users` 表本地账号，失败后允许 `admin` 使用 `.env` 的 `CHATAPI_ADMIN_PASSWORD` 作为恢复入口，成功后用独立 `CHATAPI_SESSION_SECRET` 写入 HMAC 签名 HttpOnly cookie；`GET /api/auth/session` 可读取当前 actor，`POST /api/auth/logout` 会清除 cookie；管理员接口已可通过 session actor 访问，应用 API Key 和虚拟模型 API Key 仍不能访问管理员后台。session 认证的非 GET `/api/*` 请求已执行 Origin/Referer 同源校验，Lab actor 和 API Key 请求不走 CSRF。`chatapi setup` 已生成 `CHATAPI_SESSION_SECRET`；如果老部署未配置，serve 启动会生成随机 session secret 并持久化到 `config` 表的 `security.session_secret`，Lab 使用进程内不安全默认值且不持久化。本地管理员登录已补上最小进程内失败限流，连续失败后返回 `429` 并写审计事件；认证审计 metadata 当前也已开始补齐 `auth_method`（如 `local_password` / `admin_recovery` / `oidc`）、username，以及 logout 时的 `auth_source` / role，便于管理员后台按登录方式排查问题。后续多实例部署应迁移到 Redis 或数据库限流器。
- 认证入口 schema 已开始显式暴露：`GET /api/auth/schema` 当前统一声明 session、login、logout、register、password reset、TOTP、OIDC 这一组交互式认证接口的路径、字段、是否要求已登录 session actor，以及当前运行时能力标志（Lab/Serve、OIDC 启用、注册启用、密码找回启用、邮件验证码失败次数上限等），减少前端登录页和后续 CLI/自动化工具继续把这些约束散落硬编码在本地。
...
- Kirari delegated LLM 已开始进入 ChatAPI 集成层：`internal/platform/kirari` 当前已提供 OIDC discovery、issuer allowlist、authorization URL + PKCE、authorization code exchange、ID token `nonce` 校验、UserInfo merge、refresh token、`/api/llm/meta` 和 `/api/llm/chat/completions` 原始调用能力，并带独立单元测试；在 ChatAPI 侧，当前已补上 `GET /api/user/integrations/kirari/schema`、`GET /api/user/integrations/kirari`、`GET /api/user/integrations/kirari/connect`、`GET /api/integrations/kirari/callback`、`GET /api/user/integrations/kirari/meta`、`DELETE /api/user/integrations/kirari`，以及供 Tool Call 辅助使用的 `POST /api/workspace/tool-call/assist` 和 `POST /api/workspace/tool-call/assist/stream`。当前 token 集合和 meta cache 使用受保护的 `user_configs.security.kirari` 单条 JSON 保存，access/refresh token 使用 master key 加密，普通 `/api/user/config` 不会回显这类保留配置；`meta` 接口会优先命中短 TTL 缓存，access token 临近过期时自动 refresh；`ChatCompletionsRaw` 已可把 Kirari `/api/llm/chat/completions` 的原始 HTTP response（包括 `text/event-stream`）直接向上透传，并已接到 Tool Call 流式辅助入口；Tool Call assist 内部也已从“硬编码依赖 Kirari service”进一步收口到 provider registry + 通用 chat-completions adapter：非流式执行、流式 SSE 读取、OpenAI-style delta/content 数组拼接、最终草稿解析和校验都走统一主流程，当前 Kirari 是第一个实现，后续若增加其他 delegated provider，只要实现相同 provider 接口即可复用；该入口当前也已开始写入 `user.tool_call / assist` 与 `user.tool_call / assist_stream` 审计事件，记录 provider、model、request/conversation、valid_draft、warning/validation error 计数，以及 Kirari issuer/subject 等必要元数据，不记录 prompt、token 或上游完整输出。
...
当前 Go 重构分支对这条原则保留一个窄例外：如果用户选择的是 KirariNetwork delegated upstream，那么 Tool Call 辅助可通过 `POST /api/workspace/tool-call/assist` 或 `POST /api/workspace/tool-call/assist/stream` 走后端代请求。这不是用户自定义 URL 的通用代理，而是仅针对管理员预配置 issuer/client 的 Kirari 授权链路。
...
当前 Go 重构分支里，这组数据暂时落在 `user_configs.security.kirari` 单条 JSON，而不是单独新表。这样可以复用现有 SQLite/PostgreSQL user config 存储、避免为首版连接能力单独引入 migration，同时借助 `security.*` 保留前缀避免从普通 `/api/user/config` 回显到前端。后续如果 Kirari 连接数量、审计需求或 token 生命周期管理复杂度继续上升，再考虑提升为显式表结构。
...
当前 Go 重构分支已先完成步骤 2、3、4、6、7 的最小后端闭环：

- `GET /api/user/integrations/kirari/connect` 发起 authorization code + PKCE。
- `GET /api/integrations/kirari/callback` 完成 token exchange、ID token `nonce` 校验、UserInfo merge 和加密入库。
- `GET /api/user/integrations/kirari` 返回当前连接状态、scope、过期时间和缓存元数据摘要。
- `GET /api/user/integrations/kirari/meta` 会按当前用户 delegated token 读取 `/api/llm/meta`，并做短 TTL 缓存；access token 临近过期时自动 refresh。
- `DELETE /api/user/integrations/kirari` 删除本地加密 token 和缓存。
- `POST /api/workspace/tool-call/assist` 当前会先通过 provider registry 解析出目标 delegated provider，再用统一的 chat-completions request builder 读取当前 workspace tool-call 上下文、发起非流式请求，要求上游返回结构化 JSON，再把 explanation、tool call draft、warnings、validation errors、valid_draft 和 raw_output 返回给前端；首个已接入 provider 为 `kirari`。这一步不会自动提交 tool call，也不会写入消息或修改 request 状态。
- `POST /api/workspace/tool-call/assist/stream` 走同一上下文与 provider registry，但把上游 `text/event-stream` 统一归一化为 `assist.started`、`assist.delta`、`assist.completed`、`assist.failed` 四类 SSE 事件；当前 adapter 已支持 OpenAI-style `choices[].delta.content` 字符串和 content block 数组两种常见 chunk 外形，其中 `assist.completed` 结果与非流式入口保持同构，便于后续前端统一消费。`assist.failed` 当前也会统一附带 `error`、`error_code`、`provider`、`retryable` 和 `http_status`，避免前端再依赖字符串猜测错误类型。
- ChatAPI service 层当前还已补上 `ChatCompletionsRaw`：同样复用 delegated token + 自动 refresh，并已被 Tool Call 流式辅助入口消费；上层 assist service 不再在 Kirari 分支里自己拼特殊流读取逻辑。
...
- 已支持 discovery、issuer allowlist、Auth URL + PKCE、code exchange、refresh token、ID token + `nonce` + UserInfo 合并校验，以及 delegated `/api/llm/meta` / `/api/llm/chat/completions` 调用。
- ChatAPI 集成层当前已开始复用这个包实现用户连接、token 加密存储和 meta 缓存；后续真正把 Kirari upstream 接到 Tool Call“请求大模型”和更细审计时，仍继续在 ChatAPI 集成层扩展，而不是把业务状态塞回 client 包内。
...
- `GET /api/user/identities/schema`
- `GET /api/user/app-api-keys`
...
- `POST /api/app/requests/{request_id}/complete`
- `POST /api/app/requests/{request_id}/abort`
- `GET /api/app/conversations/schema`
- `GET /api/app/conversations`
- `GET /api/app/conversations/{conversation_id}/messages`
- `GET /api/app/automation-rules`
- `PUT /api/app/automation-rules`
...
- `GET /api/admin/send-test-email/schema`
...
- 用户配置、系统配置、虚拟模型 API Key、应用 API Key、上游模型辅助的浏览器本地配置、KirariNetwork 连接、管理员用户管理。当前用户配置已先支持 `GET/POST /api/user/config` 按当前 actor 读写普通 JSON object 偏好；系统配置已先支持 `GET/POST /api/admin/config` 做表驱动持久化；管理员用户管理已先支持列表、创建、查看历史、查看/解绑 OIDC 身份、重置密码、停用用户，以及 `delete-overview + transfer-ownership/cleanup-selection + purge` 的完整物理删除链路。当前 `delete-overview` 会聚合 `delete-preview` 和 `ownership-items`，给前端单次返回 purge blocker、owned history items 和 recommended next actions。当前 purge 仅删除账号直接归属的数据行，并显式保留审计日志；如果用户仍拥有 conversations 或 uploaded images，可先通过 transfer-ownership 把历史 owner 归属迁给其他用户，或通过 cleanup-selection 清理可删历史，再执行 purge。后续仍可继续扩展成更细的按资源类型删除策略。
