...
- 管理员用户管理已落地最小接口：`GET /api/admin/users` 列出本地用户，`POST /api/admin/users` 创建本地用户并写入 Argon2id 密码 hash，`PUT /api/admin/users/{user_id}/password` 重置密码，`DELETE /api/admin/users/{user_id}` 做停用，`GET /api/admin/users/{user_id}/delete-preview` 返回物理删除预检，`POST /api/admin/users/{user_id}/purge` 在无阻断项时执行物理删除；当前 purge 会删除 identities、user configs、automation rules、应用/虚拟模型 API Key、应用 API Key 审计明细、存储配额和删除失败队列，但会保留审计日志，并在存在 owned conversations / uploaded images 时返回 `409` 阻断，避免把仍需保留的历史 owner 记录静默变成孤儿数据。这些接口仅允许 admin session 访问，并写入 `admin.user` 审计事件。
...
- 管理员用户管理 schema 已开始显式暴露：`GET /api/admin/users/schema` 当前声明本地用户列表、创建、查看历史、查看已绑定身份、删除预检、重置密码、解绑身份、停用用户、物理删除这些操作的路径、字段、默认值以及“停用”和“purge”的不同语义，避免前端或 CLI 再各自硬编码 `role` 枚举、`history.limit` 默认值和请求体字段。
...
- 用户配置、系统配置、虚拟模型 API Key、应用 API Key、上游模型辅助的浏览器本地配置、KirariNetwork 连接、管理员用户管理。当前用户配置已先支持 `GET/POST /api/user/config` 按当前 actor 读写普通 JSON object 偏好；系统配置已先支持 `GET/POST /api/admin/config` 做表驱动持久化；管理员用户管理已先支持列表、创建、查看历史、查看/解绑 OIDC 身份、重置密码、停用用户，以及 `delete-preview + purge` 两段式物理删除策略。当前 purge 仅删除账号直接归属的数据行，并显式保留审计日志；如果用户仍拥有 conversations 或 uploaded images，则预检会阻断物理删除，后续可再扩展成更细的“转移 owner / 连带清理上传文件”策略。
