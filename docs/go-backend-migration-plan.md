...
- Tool Call 辅助入口 schema 也已开始显式暴露：`GET /api/workspace/tool-call/schema` 当前同时声明 `assist-context` 和 `assist` 两类操作的查询/请求字段、返回块和交互式 actor 边界。`candidate_base_url` 只用于递归风险提示，不会落库或触发任何浏览器直连上游请求；`POST /api/workspace/tool-call/assist` 当前仅支持 `provider=kirari`，由 ChatAPI 后端代用户请求已授权的 Kirari delegated upstream，返回 explanation + tool call draft，但不会自动提交。
...
- Kirari delegated LLM 已开始进入 ChatAPI 集成层：`internal/platform/kirari` 当前已提供 OIDC discovery、issuer allowlist、authorization URL + PKCE、authorization code exchange、ID token `nonce` 校验、UserInfo merge、refresh token、`/api/llm/meta` 和 `/api/llm/chat/completions` 原始调用能力，并带独立单元测试；在 ChatAPI 侧，当前已补上 `GET /api/user/integrations/kirari/schema`、`GET /api/user/integrations/kirari`、`GET /api/user/integrations/kirari/connect`、`GET /api/integrations/kirari/callback`、`GET /api/user/integrations/kirari/meta`、`DELETE /api/user/integrations/kirari`，以及供 Tool Call 辅助使用的 `POST /api/workspace/tool-call/assist`（当前仅支持 `provider=kirari`）。当前 token 集合和 meta cache 使用受保护的 `user_configs.security.kirari` 单条 JSON 保存，access/refresh token 使用 master key 加密，普通 `/api/user/config` 不会回显这类保留配置；`meta` 接口会优先命中短 TTL 缓存，access token 临近过期时自动 refresh；`ChatCompletionsRaw` 已可把 Kirari `/api/llm/chat/completions` 的原始 HTTP response（包括 `text/event-stream`）直接向上透传，便于后续实现流式 delegated upstream；Tool Call assist 内部也已从“硬编码依赖 Kirari service”收口到 provider registry，当前 Kirari 是第一个实现，后续若增加其他 delegated provider，可在不重写 assist 主流程的前提下扩展；`assist` 接口则继续使用非流式 delegated token 请求 Kirari chat completions，并把 explanation + tool call draft 返回给前端，但不提交、不落消息、不修改 pending turn 状态。该入口当前也已开始写入 `user.tool_call / assist` 审计事件，记录 provider、model、request/conversation、valid_draft、warning/validation error 计数，以及 Kirari issuer/subject 等必要元数据，不记录 prompt、token 或上游完整输出。
...
当前 Go 重构分支对这条原则保留一个窄例外：如果用户选择的是 KirariNetwork delegated upstream，那么 Tool Call 辅助可通过 `POST /api/workspace/tool-call/assist` 走后端代请求。这不是用户自定义 URL 的通用代理，而是仅针对管理员预配置 issuer/client 的 Kirari 授权链路。
...
- 当前 Go 重构分支还额外提供 `POST /api/workspace/tool-call/assist` 作为例外入口，但它不是通用服务端上游代理：目前只允许 `provider=kirari`，由后端使用该用户已授权的 Kirari delegated token 请求 `/api/llm/chat/completions`，返回 explanation、tool call draft、validation errors 和原始输出摘要。
...
- `POST /api/workspace/tool-call/assist` 在 `provider=kirari` 时会读取当前 workspace tool-call 上下文，向 Kirari `/api/llm/chat/completions` 发起非流式请求，要求上游返回结构化 JSON，再把 explanation、tool call draft、warnings、validation errors、valid_draft 和 raw_output 返回给前端；这一步不会自动提交 tool call，也不会写入消息或修改 request 状态。
- ChatAPI service 层当前还已补上 `ChatCompletionsRaw`：同样复用 delegated token + 自动 refresh，但返回原始 `*http.Response`，用于后续把 Kirari 流式响应接到 SSE/工作台辅助等场景，而不是在 service 层强行解 JSON。
