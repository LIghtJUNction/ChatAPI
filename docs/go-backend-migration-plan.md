...
- 登录、登出、session、注册、密码重置、TOTP、OIDC RP 登录。当前已先落地本地 users 表登录、`.env` admin 恢复登录、登出、session、注册、邮箱验证码发送、密码重置、TOTP 最小闭环、OIDC RP 最小登录闭环和用户侧 OIDC 身份查看/解绑；GeeTest 校验、验证码限流/尝试次数限制和 OIDC 绑定发起流程仍待补齐。
