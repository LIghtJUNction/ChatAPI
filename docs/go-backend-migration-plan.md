...
- `POST /api/workspace/tool-call/assist/stream` 走同一上下文与 provider registry，但把上游 `text/event-stream` 统一归一化为 `assist.started`、`assist.delta`、`assist.completed`、`assist.failed` 四类 SSE 事件；当前 adapter 已支持 OpenAI-style `choices[].delta.content` 字符串和 content block 数组两种常见 chunk 外形，其中 `assist.completed` 结果与非流式入口保持同构，便于后续前端统一消费。
