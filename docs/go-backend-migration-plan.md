...
- 管理员用户管理已落地最小接口：`GET /api/admin/users` 列出本地用户，`POST /api/admin/users` 创建本地用户并写入 Argon2id 密码 hash，`PUT /api/admin/users/{user_id}/password` 重置密码，`DELETE /api/admin/users/{user_id}` 做停用，`GET /api/admin/users/{user_id}/delete-preview` 返回物理删除预检，`POST /api/admin/users/{user_id}/transfer-ownership` 可把阻断 purge 的 conversations / uploaded_images / 删除失败队列 owner 转移给另一个用户，`GET /api/admin/users/{user_id}/ownership-items` 可列出当前仍归属于该用户的 conversations 和 uploads，`POST /api/admin/users/{user_id}/transfer-ownership-selection` 可只迁移选中的 conversation ids / upload filenames，`POST /api/admin/users/{user_id}/purge` 在无阻断项时执行物理删除；当前 purge 会删除 identities、user configs、automation rules、应用/虚拟模型 API Key、应用 API Key 审计明细、存储配额和删除失败队列，但会保留审计日志，并在存在 owned conversations / uploaded images 时返回 `409` 阻断，避免把仍需保留的历史 owner 记录静默变成孤儿数据。ownership transfer 当前不会转移账号直接归属的 key/config，而是只处理 purge blocker 和紧邻的存储归属；如果 source 有 quota override 且 target 没有，则 quota override 一并迁移，否则保留 target quota 并删除 source override。局部 transfer 当前不会触碰 quota override，只做精细的历史 owner 收口。这些接口仅允许 admin session 访问，并写入 `admin.user` 审计事件。
...
- 管理员用户管理 schema 已开始显式暴露：`GET /api/admin/users/schema` 当前声明本地用户列表、创建、查看历史、查看已绑定身份、删除预检、全量 ownership transfer、ownership item 列表、局部 ownership transfer、重置密码、解绑身份、停用用户、物理删除这些操作的路径、字段、默认值以及“停用”“transfer”“purge”的不同语义，避免前端或 CLI 再各自硬编码 `role` 枚举、`history.limit` 默认值和请求体字段。
