...
- `POST /api/workspace/tool-call/assist/stream` 在 `provider=kirari` 时走同一上下文与 delegated token 链路，但把上游 `text/event-stream` 归一化为 `assist.started`、`assist.delta`、`assist.completed`、`assist.failed` 四类 SSE 事件，避免前端直接绑定 Kirari/OpenAI 风格 chunk 结构；其中 `assist.completed` 结果与非流式入口保持同构，便于后续前端统一消费。
- ChatAPI service 层当前还已补上 `ChatCompletionsRaw`：同样复用 delegated token + 自动 refresh，并已被 Tool Call 流式辅助入口消费，而不是在 service 层强行解 JSON。
