...
- 登录、登出、session、注册、密码重置、TOTP、OIDC RP 登录。当前已先落地本地 users 表登录、`.env` admin 恢复登录、登出、session、注册、邮箱验证码发送、密码重置、TOTP 最小闭环、OIDC RP 登录/绑定闭环、用户侧 OIDC 身份查看/解绑，以及验证码后台过期清理任务；GeeTest 校验仍待补齐。
